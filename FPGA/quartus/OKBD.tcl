...
if {[is_project_open]} {
	if {[string compare $quartus(project) "OKBD"]} {
		puts "Project OKBD is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists OKBD]} {
		project_open -revision OKBD OKBD
	} else {
		project_new -revision OKBD OKBD
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name SYSTEMVERILOG_FILE ../src/OKBD.sv
