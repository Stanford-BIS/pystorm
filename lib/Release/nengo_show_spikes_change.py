...
class BrainDropSpikes(nengo.Process):
    def __init__(self, n_neurons):
        ...
        super(BrainDropSpikes, self).__init__(default_size_in=1, default_size_out=n_neurons)
    ...
    def make_step(self, size_in, size_out, dt, rng):
        ...
        # store the accululator value and the last wall-clock time
        data = np.array([0.0, timeit.default_timer()])
        
        def step_braindrop_spikes(t, x):
...
with model:
    ...
    stim = nengo.Node(0)
    nengo.Connection(stim, braindrop)
