...
DEFAULT_TEST_TIME = 3.0 # time to collect overflow data
...
def test_syn(syn_idx, test_time, slop_time):
    ...
    # validate overflow predicted rate
    low_idx = np.nonzero(SPIKE_GEN_RATES <= max_rate)[0][-1]
    observable_rate_diff = FIFO_BUFFER_SIZE / test_time + VALIDATE_HIGH_BUF_RATE
    high_idx = np.nonzero(SPIKE_GEN_RATES > max_rate + observable_rate_diff)[0][0]
    low_rate = SPIKE_GEN_RATES[low_idx]
    high_rate = SPIKE_GEN_RATES[high_idx]
    ...
    return max_rate, (low_rate, low_overflows, high_rate, high_overflows)
...
def plot_data(max_rates_2, check_data):
    """Plot the data"""
    low_rates = check_data[:, 0]
    low_rate_overflows = check_data[:, 1]
    high_rates = check_data[:, 2]
    high_rate_overflows = check_data[:, 3]
    ...
    axs[0].plot(low_rate_overflows, 'o', label="max non-overflow")
    axs[0].plot(high_rate_overflows, 'o', label="min overflow")
...
def check_synapse_max_rates(parsed_args):
    ...
    if use_saved_data:
        ...
        check_data = load_txt_data(DATA_DIR + "check_data.txt")
    else:
        max_rates_2 = np.zeros(SYN_N)
        check_data = np.zeros((SYN_N, 4))
        build_net()
        set_analog()
        set_hal()
        start_time = get_time()
        for syn_idx in range(SYN_N):
            ...
            max_rates_2[syn_idx], check_data[syn_idx] = test_syn(
                syn_idx, DEFAULT_TEST_TIME, DEFAULT_SLOP_TIME)
        ...
        np.savetxt(DATA_DIR + "check_data.txt", check_data)

    plot_data(max_rates_2, check_data)
    plt.show()
