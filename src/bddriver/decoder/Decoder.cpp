...
void Decoder::RunOnce() {
  ...
  if (in_buf_->TotalSize() > driverpars::READ_LAG_WARNING_SIZE) { 
    cout << "WARNING: bddriver::Decoder (upstream data processing) is too slow. Running " << in_buf_->TotalSize() / driverpars::READ_SIZE << " comm reads behind." << endl;
  }
...
void Decoder::Decode(std::unique_ptr<std::vector<DecInput>> input) {
    ...
    for (unsigned int word_idx = start; word_idx < end; word_idx += BYTES_PER_WORD) { // iterating by 4!!
      ...
      if (ep_code == bd_pars_->UpEPCodeFor(bdpars::FPGAOutputEP::UPSTREAM_HB_LSB)) {
        ...
        if (this_HB - curr_HB_recvd_ != curr_HB_recvd_ - last_HB_recvd_) { 
          cout << "WARNING: bddriver::Decoder::Decode: upstream USB is too slow. Possibly missed an upstream HB. Jump was " <<
  ...
  if (!had_nop) {
    cout << "WARNING: bddriver::Decoder::Decode: upstream USB is too slow. Didn't receive any nops in a read, FPGA possibly stalled BD, lost data" << endl;
  }
}
