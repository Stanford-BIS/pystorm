...
template <class T>
bool MutexBuffer<T>::WaitForHasAtLeast(std::unique_lock<std::mutex>* lock, unsigned int try_for_us,
  ...
  if (!HasAtLeast(multiple)) {
    if (try_for_us == 0) {
      ...
      bool success = just_pushed_.wait_for(*lock, std::chrono::duration<unsigned int, std::micro>(try_for_us),
...
template <class T>
bool MutexBuffer<T>::WaitForHasRoomFor(std::unique_lock<std::mutex>* lock, unsigned int input_len,
  ...
  if (!HasRoomFor(input_len)) {
    if (try_for_us == 0) {
      ...
      bool success = just_popped_.wait_for(*lock, std::chrono::duration<unsigned int, std::micro>(try_for_us),
...
template <class T>
std::pair<const T*, unsigned int> MutexBuffer<T>::LockFront(unsigned int max_to_pop, unsigned int try_for_us,
                                                            unsigned int multiple) {
