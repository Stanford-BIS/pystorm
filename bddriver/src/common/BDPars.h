...
enum RegWordId {
  ADC_word,
  TOGGLE_PRE_FIFO_word,
  TOGGLE_POST_FIFO0_word,
  TOGGLE_POST_FIFO1_word,
  NeuronDumpToggle_word,
  DAC0_word,
  DAC1_word,
  DAC2_word,
  DAC3_word,
...
class BDPars {
  public:
    ...
    inline FHRoute HornRoute(HornLeafId leaf) const { return HornRoute(HornIdx(leaf)); }
    inline FHRoute FunnelRoute(FunnelLeafId leaf) const { return FunnelRoute(FunnelIdx(leaf)); }

    inline unsigned int HornIdx(HornLeafId leaf) const { return horn_leaf_name_to_idx_.at(leaf); }
    inline unsigned int FunnelIdx(FunnelLeafId leaf) const { return funnel_leaf_name_to_idx_.at(leaf); }
    ...
    std::vector<FunnelLeafId> FunnelLeafIds() const 
    ...
    inline const WordStructure * Word(RegWordId object)    const { return &(reg_.at(object).word_structure); }
    ...
    HornLeafId DACSignalIdToDACRegisterId(DACSignalId);
  ...
  private:
    ...
    std::unordered_map<RegWordId, RegInfo> reg_;
