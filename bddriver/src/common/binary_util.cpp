...
template <class TIN, class TOUT>
inline std::pair<unsigned int, TOUT> DecodeFH(TIN input, const std::vector<TIN>& leaf_routes,
...
std::pair<std::vector<TOUT>, unsigned int> SerializeWord2(TIN input, unsigned int input_width) {
  ...
  return {{unpacked[0], unpacked[1]}, half_chunk_width};
}
...
std::pair<std::vector<TOUT>, unsigned int> SerializeWord4(TIN input, unsigned int input_width) {
  ...
  return {{retval01[0], retval01[1], retval23[0], retval23[1]}, chunk_width01};
}

template <class TIN, class TOUT>
std::pair<std::vector<TOUT>, unsigned int> SerializeWords(const std::vector<TIN>& inputs, unsigned int input_width,
...
TOUT DeserializeWord2(std::vector<TIN> inputs, unsigned int output_width) {
  ...
  uint64_t packed_word = PackV<TIN, TOUT>({inputs[0], inputs[1]}, {half_width + remainder, half_width});
...
template <class TIN, class TOUT>
std::pair<std::vector<TOUT>, std::vector<TIN> > DeserializeWords(const std::vector<TIN>& inputs,
                                                                 unsigned int output_width,
                                                                 unsigned int deserialization) {
  ...
  if (deserialization == 1) {
    ...
    for (unsigned int i = 0; i < inputs.size() / 4; i++) {
      deserialized_words.push_back(DeserializeWord4<TIN, TOUT>(
          {inputs[4 * i + 0], inputs[4 * i + 1], inputs[4 * i + 2], inputs[4 * i + 3]}, output_width));
    }
